...
describe('inspect', () => {
  ...
  it('var boolean', () => {
    ...
    expect(printQuickInfo(info)).toEqual(['var klm: boolean', '', 'klm is a great boolean'].join('\n'));
  });
  ...
  it('const string', () => {
    ...
    expect(printQuickInfo(info)).toEqual(['const abc: "hello"', '', 'abc is a great string'].join('\n'));
  });
  ...
  it('std constructor with override', () => {
    ...
    expect(printQuickInfo(info)).toEqual(['var Map: MapConstructor', 'new () => Map<any, any> (+2 overloads)'].join('\n'));
  });
